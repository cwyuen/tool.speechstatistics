...
public interface FrequencyWordSourceDao  extends GraphRepository<FrequencyWordSource>{
	...
	@Query("MATCH (n1:FQ_WORD) -[r1:FQ_SOURCE]- (n2:FQ_FILE) "
			...
			+ "and id(n2) = {1} "
			+ "return r1 limit 1")
	public FrequencyWordSource findFrequencyWordSources(long wordId, long fileId);
	
	@Query("MATCH (n1:FQ_WORD) -[r1:FQ_SOURCE]- (n2:FQ_FILE) "
			+ "where id(n1) = {0} "
			+ "return r1")
	public List<FrequencyWordSource> findFrequencyWordSources(long wordId);
	
	
	@Query("MATCH (n1:FQ_WORD) -[r1:FQ_SOURCE]- (n2:FQ_FILE) "
			+ "return r1")
	public List<FrequencyWordSource> findFrequencyWordSources();
}
