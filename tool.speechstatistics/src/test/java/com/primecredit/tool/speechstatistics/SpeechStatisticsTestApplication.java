...
public class SpeechStatisticsTestApplication {
	...
	public void testNaturalLang() {
		...
		try {
			result = result && speechStatisticsService.saveNaturalLang("Apple", "ORGANIZATION", "ORGANIZATION.txt", 1);
			result = result && speechStatisticsService.saveNaturalLang("Apple", "ORGANIZATION", "ORGANIZATION.txt", 5);
	...
	public void testFrequencyWord() {
		...
		try {
			result = result && speechStatisticsService.statisticsFrequencyWord("Fff.txt", speechTexts);
		} catch (Exception e) {
			logger.error("Exception - SpeechStatisticsTestApplication.testNaturalLang: " + e.getMessage());
		}
